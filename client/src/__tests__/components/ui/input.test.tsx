...
vi.mock("../../../components/ui/form/form-error", () => ({
  default: ({ children, id }: { children: React.ReactNode; id?: string }) =>
    children ? (
      <div data-testid="form-error" id={id}>
        {children}
      </div>
    ) : null,
}));
...
describe("Input Component", () => {
  ...
  it("applies different size styles based on dim prop", () => {
    const { rerender } = render(
      <Input name="test" dim="sm" data-testid="input" />,
    );
    let input = screen.getByTestId("input");
    expect(input.className).toContain("px-1 py-0 text-sm");

    rerender(<Input name="test" dim="md" data-testid="input" />);
    input = screen.getByTestId("input");
    expect(input.className).toContain("px-2 py-1 text-base");

    rerender(<Input name="test" dim="lg" data-testid="input" />);
    input = screen.getByTestId("input");
  ...
  it("applies custom className", () => {
    render(<Input name="test" className="custom-class" data-testid="input" />);
    const input = screen.getByTestId("input");
  ...
  it("updates value when input changes", () => {
    render(<Input name="test" data-testid="input" />);
    const input = screen.getByTestId("input");
